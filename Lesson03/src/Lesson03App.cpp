...
public:
	...
	gl::VboRef			mPositionData;
	geom::BufferLayout	mPositionLayout;

	gl::VboRef			mColorData;
	geom::BufferLayout	mColorLayout;
...
void Lesson03App::setup()
{
	...
	mCamera.lookAt(vec3(2, 2, 2), vec3(0), vec3(0, 1, 0));
	...
	vector<vec3> cVertices =
	{
		...
		vec3(1.0f, 1.0f, 1.0f),	vec3(-1.0f, 1.0f, -1.0f), vec3(-1.0f, 1.0f, 1.0f),
		vec3(1.0f, 1.0f, 1.0f),	vec3(-1.0f, 1.0f, 1.0f), vec3(1.0f, -1.0f, 1.0)
	};
	...
	mColorLayout.append(geom::Attrib::COLOR, 4, sizeof(vec4), 0);

	mCubeMesh = gl::VboMesh::create(12, GL_TRIANGLES, { { mPositionLayout, mPositionData }, { mColorLayout, mColorData } });
